...
public class Tyre {
    ...
    void updateForces(double Fz, double kappa, double alpha) {
        ...
        // Composite slip
        double sigma = (Math.PI * Math.pow(ap, 2) / (8 * mu0 * Fz) ) * Math.sqrt(Math.pow(Ks * Math.tan(alpha), 2) + Math.pow(Kc * kappa/(1 - kappa), 2));

        // System.out.println((Math.PI * Math.pow(ap, 2) / (8 * mu0 * Fz) ) + " * " + Math.pow(Ks, 2) + " + " + Math.pow(2, 2));
    ...
    2 - Mz ;)
    */
    public double[][] getCharacteristic(double[] longSlip, double[] latSlip, double Fz)
    {
        // Longitudinal slip
        assert(longSlip.length == latSlip.length);
        int n = longSlip.length;

        double[][] res = new double[3][n];
        for(int i = 0; i < n; ++i) {
            this.updateForces(Fz, longSlip[i], latSlip[i]);
            res[0][i] = Fx; res[1][i] = Fy; res[2][i] = Mz;
        }
        return res;
